defmodule PipelineTest do

  use ExUnit.Case
  alias Pallium.Examples.Pipeline.{EventHandler, Predictor}
  test "execution prediction pipeline" do
    raw = "data:image/png;base64,"

    prediction = []

    {:ok, event_handler} = EventHandler.start_link()
    {:ok, predictor} = EventHandler.start_link()
    GenStage.sync_subscribe(predictor, to: event_handler)
    assert CV.raw_to_rgb(raw) == rgb
  end
end
